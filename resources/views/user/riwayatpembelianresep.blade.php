            ...
            <div class="card-body d-flex justify-content-between py-4">
                ...
                <div class="col-md-7 me-4" style="width: 70%">
                    <div class="card-body p-3">
                        <div class="col">
                            @if (!$pesananresep->count())
                                <p>Belum ada Riwayat Pembelian</p>
                            @else
                                @foreach ($pesananresep as $pr)
                                    {{-- Cards Pembelian --}}
                                    <div class="card mt-3">
                                        <div class="row g-0">
                                            <div class="col-md-8">
                                                <div class="card-body">
                                                    <p class="card-text">ID Pesanan Resep : {{ $pr->id }}</p>
                                                    <p class="card-text">Total : Rp.{{ $pr->total_harga }},00</p>
                                                    <p class="card-text">Status : {{ $pr->status }}</p>
                                                    <p class="card-text">Obat - Obat : {{ $pr->obat_obat }}</p>
                                                    <img src="{{ asset('storage/'.$pr->image) }}" alt="r" style="display: block; width: 300px">
                                                    {{-- <a href="rsp/{{ $pr->id }}" class="btn btn-success">Detail</a> --}}
                                                </div>
                                            </div>
                                        </div>
                                    </div>
